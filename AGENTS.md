...
- [x] **模式A三格式兼容性修复 6 项（两入口同步，代码已改 node --check + grep 对称性通过，待部署验证）：P0 safeJsonBody 空 CT 放行 / P1 STRICT_MODEL_MATCH 严格模型 404 开关（默认关）/ Low2 stop_sequence 映射 / Low3 count_tokens 路径匹配 / Low4 流式删 CF 非标准字段 / Low5 Responses 回显请求值；Low1 top_k 系设计性丢弃不修**
